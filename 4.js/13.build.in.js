/* 
    1. JS 객체의 종류
        ...
        1) 표준 빌트인 객체
            - ECMAScript 사양에 정의된 객체로 전역에서 사용할수 있는 공통의 기능을 제공한다.
...
/* 
    2. 전역 객체
        ...
        - 표준 빌트인 객체, 호스트 객체, var 키워드 선언 전역변수와 전역함수를 프로퍼터로 갖는다.
...
// 키워드를 생략하면 window 객체의 프로퍼티로 할당이 된다.
a = 10
var b = 6

function foo() {
  var c = 0 // var 키워드는 함수 레벨 스코프(유효 범위)를 지원하기 때문에 window 객체에 할당 안한다.
  d = 5
}
foo()
...
// window 객체에 호스트 객체가 포함되어 있다.
const app = window.document.querySelector('#app')

/* 
    Date 객체 : 날짜와 시간을 다루는 객체
        => 생성자 함수이다. => new 키워드로 함께 호출하여 인스턴스를 생성해서 사용한다.
*/
...
let day = today.getDay() // day 일요일(0) ~ 토요일(6)
...
// Math.random() : 0~1 사이의 실수 => ex) 0.00102031, 0.999999991 => 진짜 랜덤이 아니다.
